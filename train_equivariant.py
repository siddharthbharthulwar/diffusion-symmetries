...
class PointDiffusion:
    ...
    def train_step(self, x_0, k_steps=1, lambda_equiv=10):
        ...
        # Only apply equivariance loss for masked timesteps
        equivariance_loss = torch.tensor(0.0).to(self.device)
        mask = t >= 0
        if mask.any():
            # Get points at final timesteps
            x_t_final = x_t[mask]
            t_final = t[mask]
            
            # Sample random rotation angle
            angles = torch.rand(mask.sum(), device=self.device) * 2 * torch.pi
            
            # Path 1: Rotate -> Denoise k times
            x_path1 = sample_rotation(angles, x_t_final, device=self.device)
            for _ in range(k_steps):
                noise_pred = self.model(x_path1, t_final.float() / self.n_steps)
                alpha = self.alpha[t_final].view(-1, 1)
                alpha_bar = self.alpha_bar[t_final].view(-1, 1)
            ...
            x_path2 = x_t_final
            for _ in range(k_steps):
                noise_pred = self.model(x_path2, t_final.float() / self.n_steps)
                alpha = self.alpha[t_final].view(-1, 1)
                alpha_bar = self.alpha_bar[t_final].view(-1, 1)
