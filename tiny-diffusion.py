...
if __name__ == "__main__":
    # Create synthetic 2D point data with radius 0.8
    points = circle_dataset(n=1000, radius=5, noise=0.1, angle_intervals=[(0, 30), (60, 90), (120, 150), (180, 210), (240, 270), (300, 330)])

    points = circle_dataset(n=1000, radius=5, noise=0.1, angle_intervals=[(0, 360)])
    ...
    plt.scatter(points_np[:, 0], points_np[:, 1], s=1)
    plt.xlim(xlim)
    plt.ylim(ylim)
    ...
    diffusion = PointDiffusion(point_dim=2, n_steps=num_steps, device="cuda")
    ...
    for epoch in range(n_epochs):
        loss = diffusion.train_step(points)
        if epoch % 100 == 0:
    ...
    # Plot score errors
    fig = plot_score_errors(diffusion, timestep=timestep, R=5, sigma=0.025, xlim=xlim, ylim=ylim)
    plt.savefig(f'score_errors.png', bbox_inches='tight', dpi=300)
    plt.close()

    # Generate points from timestep
    points = diffusion.sample(n_points=1000, shape=2, t=timestep)
    points_np = points.cpu().numpy()
    
    # Create grid for score fields
    x = np.linspace(xlim[0], xlim[1], 50)
    y = np.linspace(ylim[0], ylim[1], 50)
    xx, yy = np.meshgrid(x, y)
    grid_points = np.stack([xx.flatten(), yy.flatten()], axis=1)
    
    # Compute true score field
    true_scores = true_score(grid_points, R=5, sigma=0.025)
    true_score_x = true_scores[:, 0].reshape(50, 50)
    true_score_y = true_scores[:, 1].reshape(50, 50)
    
    # Plot true score field
    plt.figure(figsize=(10, 10))
    plt.quiver(xx, yy, true_score_x, true_score_y, alpha=0.5, color='red', width=0.005)
    plt.title('True Score Field')
    plt.xlim(xlim)
    plt.ylim(ylim)
    plt.grid(True)
    plt.savefig("true_score_field.png", bbox_inches='tight', dpi=300)
    plt.close()
    
    # Plot learned score field with points overlaid
    xx, yy, score_x, score_y = compute_score_field(diffusion, 
                                                  timestep=timestep,
                                                  x_range=xlim,
                                                  y_range=ylim)
    
    plt.figure(figsize=(10, 10))
    # Plot score field vectors
    plt.quiver(xx, yy, score_x, score_y, alpha=0.5, color='red', width=0.005)
    # Overlay points
    plt.scatter(points_np[:, 0], points_np[:, 1], s=10, c='blue', alpha=0.9, label='Generated Points')
    plt.title(f'Learned Score Field with Generated Points (t={timestep})')
    plt.xlim(xlim)
    plt.ylim(ylim)
    plt.grid(True)
    plt.legend()
    plt.savefig("learned_score_field_with_points.png", bbox_inches='tight', dpi=300)
    plt.close()

    # Plot just the generated points
    plt.figure(figsize=(8, 8))
    plt.scatter(points_np[:, 0], points_np[:, 1], s=1)
